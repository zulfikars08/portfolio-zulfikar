...
type NavbarProps = {
  t: Record<'about' | 'skills' | 'projects' | 'contact' | 'hire', string>;
  locale: Locale;
...
export function Navbar({ t, locale, setLocale, theme, setTheme }: NavbarProps) {
  ...
  return (
    <header className="fixed left-0 right-0 top-4 z-50 px-4">
      <div className={isDark ? 'mx-auto flex max-w-6xl items-center justify-between gap-3 rounded-full border border-white/10 bg-slate-950/75 px-4 py-3 shadow-2xl shadow-black/30 backdrop-blur-xl sm:px-6' : 'mx-auto flex max-w-6xl items-center justify-between gap-3 rounded-full border border-slate-200 bg-white/80 px-4 py-3 shadow-2xl shadow-slate-200/70 backdrop-blur-xl sm:px-6'}>
        ...
        <nav aria-label="Primary navigation" className="hidden items-center gap-1 lg:flex">
          {navItems.map((item) => (
            <a
              ...
              key={item.href}
              href={item.href}
              className={isDark ? 'rounded-full px-4 py-2 text-sm font-semibold text-slate-300 transition hover:bg-white/10 hover:text-white' : 'rounded-full px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-950/5 hover:text-slate-950'}
            >
              {t[item.key]}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            href="#contact"
            className="hidden rounded-full bg-cyan-300 px-4 py-2 text-sm font-bold text-slate-950 transition hover:-translate-y-0.5 hover:bg-cyan-200 sm:inline-flex"
          >
            {t.hire}
          </a>
        </div>
