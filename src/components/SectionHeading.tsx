...
export function SectionHeading({ eyebrow, title, description, centered = false, theme }: SectionHeadingProps) {
  ...
  return (
    <div className={centered ? 'mx-auto mb-12 max-w-3xl text-center' : 'mb-12 max-w-3xl'}>
      <p className={isDark ? 'mb-4 inline-flex rounded-full border border-cyan-400/25 bg-cyan-400/10 px-4 py-2 text-xs font-bold uppercase tracking-[0.22em] text-cyan-200' : 'mb-4 inline-flex rounded-full border border-cyan-300 bg-cyan-50 px-4 py-2 text-xs font-bold uppercase tracking-[0.22em] text-cyan-700'}>
        {eyebrow}
      </p>
