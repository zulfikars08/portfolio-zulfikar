...
type ProjectsText = {
  ...
  labels: Record<'overview' | 'problem' | 'solution' | 'role' | 'stack' | 'features' | 'challenges' | 'decisions' | 'impact' | 'confidentiality' | 'workflow', string>;
};
              ...
              <article key={project.title} className={isDark ? 'overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.06] shadow-2xl shadow-black/10 backdrop-blur transition hover:-translate-y-1 hover:border-cyan-300/40' : 'overflow-hidden rounded-[2rem] border border-slate-200 bg-white/80 shadow-2xl shadow-slate-200/70 backdrop-blur transition hover:-translate-y-1 hover:border-cyan-300/70'}>
                <div className="grid gap-0 md:grid-cols-[0.9fr_1.1fr]">
                  <div className={isDark ? 'border-b border-white/10 bg-slate-950/35 p-6 md:border-b-0 md:border-r' : 'border-b border-slate-200 bg-slate-50/80 p-6 md:border-b-0 md:border-r'}>
                    ...
                    {project.demoVideo ? <video controls preload="metadata" className="mb-4 aspect-video w-full rounded-2xl object-cover"><source src={project.demoVideo} /></video> : project.demoGif ? <Image unoptimized src={project.demoGif} alt={`${project.title} demo`} width={1280} height={720} className="mb-4 aspect-video w-full rounded-2xl object-cover" /> : project.screenshot ? <Image src={project.screenshot} alt={`${project.title} application screenshot`} width={1280} height={720} className="mb-4 aspect-video w-full rounded-2xl object-cover object-top" /> : null}
                  ...
                  <div className="p-6 sm:p-7">
                    ...
                    {project.status ? <p className="mt-3 text-sm font-bold text-amber-300">{project.status}</p> : null}
